...
Uses AST analysis and AI for intelligent code processing.
"""
...
try:
    from gemini_integration import GeminiAgent, ContentType
    HAS_GEMINI = True
except ImportError:
    HAS_GEMINI = False
...
class SequentialStep:
    """Represents a step in sequential code reconstruction"""
    step_number: int
    description: str
    code_pattern: str
    explanation: str
...
class ASTAnalyzer:
    ...
    def analyze_code_structure(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Categorize elements
            self._categorize_elements(code, analysis)
    ...
    def _categorize_elements(self, code: str, analysis: Dict[str, Any]):
        """Categorize code elements as deterministic or user-defined"""
        lines = code.split('\n')
        
        for line in lines:
            # Find deterministic elements (keywords, built-ins)
            words = re.findall(r'\b\w+\b', line)
            for word in words:
                if word in self.deterministic_elements:
                    analysis["deterministic_elements"].append(word)
                elif word not in self.deterministic_elements:
                    # Check if it's user-defined
                    if (any(word == func["name"] for func in analysis["functions"]) or
                        any(word == cls["name"] for cls in analysis["classes"]) or
                        any(word == var["name"] for var in analysis["variables"])):
                        analysis["user_defined_elements"].append(word)
...
class PartialDeletionGenerator:
    ...
    def generate_partial_deletions(self, code: str, max_deletions: int = 3) -> List[Dict[str, Any]]:
        ...
        # Use Gemini for intelligent deletion identification if available
        if self.gemini_agent:
            gemini_deletions = self.gemini_agent.identify_key_deletions(code)
            for deletion in gemini_deletions[:max_deletions]:
                deletions.append(self._create_partial_deletion_card(
                    code, deletion["text"], deletion["explanation"]
                ))
        else:
...
class SequentialDeletionGenerator:
    ...
    def generate_sequential_deletion(self, code: str, task_description: str = "") -> Dict[str, Any]:
        ...
        if self.gemini_agent:
            # Use Gemini for intelligent sequential breakdown
            sequential_data = self.gemini_agent.generate_sequential_explanation(code, task_description)
            return self._create_sequential_card(code, sequential_data)
        else:
            # Fallback to heuristic breakdown
            return self._create_heuristic_sequential_card(code, task_description)
    
    def _create_sequential_card(self, code: str, sequential_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a sequential deletion card from Gemini analysis"""
        lines = code.split('\n')
        sequential_content = []
        
        # Create task description
        task = sequential_data.get("task", "Complete the code implementation")
        explanation = sequential_data.get("explanation", "This code performs a specific task")
        legend = sequential_data.get("legend", {})
        
        # Build sequential cloze inputs
...
~"""
        ...
        return {
            ...
            "back": "",
            "content": content,
            "hint": "Complete the code step by step. Follow the task description and use the legend for guidance.",
            "tags": "code,sequential_deletion,step_by_step,programming",
...
# Factory function
def create_cloze_system(gemini_agent: Optional[GeminiAgent] = None) -> AdvancedClozeSystem:
