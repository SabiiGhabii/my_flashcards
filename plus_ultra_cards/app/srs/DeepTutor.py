...
import numpy as np
# Prefer gym, fallback to gymnasium
try:
    import gym
...
# rllab may be unavailable in production; guard its imports
try:
    from rllab.policies.categorical_gru_policy import CategoricalGRUPolicy
    from rllab.baselines.linear_feature_baseline import LinearFeatureBaseline
    from rllab.algos.trpo import TRPO
    from rllab.misc.overrides import overrides
except Exception:  # fallbacks so module can be imported without rllab
    CategoricalGRUPolicy = None  # type: ignore
    LinearFeatureBaseline = None  # type: ignore
    TRPO = None  # type: ignore
    def overrides(f):
        return f

from matplotlib import pyplot as plt
# %matplotlib inline  # disabled for library import
...
class StudentEnv(gym.Env):
  ...
  def __init__(self, n_items=10, n_steps=100, discount=1., sample_delay=None, reward_func='likelihood'):
    ...
    # Use float32 to avoid precision cast warnings from gymnasium
    self.observation_space = spaces.Box(np.zeros(4, dtype=np.float32), np.array([n_items-1, 1, sys.maxsize, sys.maxsize], dtype=np.float32), dtype=np.float32)
...
def make_rl_student_env(env):
  ...
  env.observation_space = spaces.Box(
    np.concatenate((np.ones(env.n_item_feats, dtype=np.float32) * -np.finfo(np.float32).max, np.zeros(3, dtype=np.float32))),
    np.concatenate((np.ones(env.n_item_feats + 2, dtype=np.float32) * np.finfo(np.float32).max, np.ones(1, dtype=np.float32))),
    dtype=np.float32
  )
  ...
  return env
# rllab GymEnv wrapper (not used in production with SB3); keep for reference but guard import
try:
    from rllab.envs.gym_env import *  # type: ignore
except Exception:
    GymEnv = object  # type: ignore
    def convert_gym_space(x):
        return x
    class logger:
        @staticmethod
        def log(*args, **kwargs):
    ...
    class Serializable:
        @staticmethod
        def quick_init(*args, **kwargs):
            pass
    class NoVideoSchedule: pass
    class CappedCubicVideoSchedule: pass
...
class MyGymEnv(GymEnv):
    def __init__(self, env, record_video=False, video_schedule=None, log_dir=None, record_log=False,
        ...
        self._horizon = getattr(self.env, 'n_steps', None)
...
class DummyTutor(Tutor):
  ...
  def reset(self):
    pass
# Legacy rllab RLTutor kept for reference; not used in SB3 path
class LoggedTRPO:
  pass
...
class RLTutor(Tutor):
  ...
  def train(self, gym_env, n_eps=10):
    # No-op in production SB3 integration; training handled by dt_adapter.DeepTutorAdapter
    return []
...
if __name__ == '__main__':
  # Original heavy demo guarded to avoid running on import
  pass
...
def run_eps(agent, env, n_eps=100):
  ...
  return tot_rew
# The following heavy demo block is disabled to avoid running at import time.
# It is preserved below but commented out intentionally.

''' DEMO BLOCK DISABLED: preserved original paper demo below to avoid side effects on import.
